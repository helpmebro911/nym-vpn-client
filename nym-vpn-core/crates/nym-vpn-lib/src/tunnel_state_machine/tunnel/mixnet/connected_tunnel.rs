...
const PROCESSOR_DISCONNECT_TIMEOUT: Duration = Duration::from_secs(10);
...
pub async fn start_mixnet_tunnel(
    ...
    let processor_config = MixnetProcessorConfig::new(
        assigned_addresses.exit_mix_addresses,
    ...
    let mixnet_client_sender = mixnet_client
        .lock()
        .await
        .as_ref()
    ...
    connection_monitor.start(
        ...
        assigned_addresses.interface_addresses,
        assigned_addresses.exit_mix_addresses.into(),
        &task_manager,
    );
...
impl TunnelHandle {
    ...
    pub async fn cancel(&mut self) {
        ...
        if let Some(processor_disconnected) = self.processor_disconnected.take() {
            tokio::time::timeout(PROCESSOR_DISCONNECT_TIMEOUT, processor_disconnected)
                .await
                .unwrap_or_else(|_| {
                    tracing::error!(
