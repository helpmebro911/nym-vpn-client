    ...
    S: VpnStorage,
{
    ...
    fn handle_register_offline_watch(
        &mut self,
        ...
        self.offline_watch
            ...
            .map_err(|err| {
                AccountCommandError::internal(format!("Failed to register offline watch: {err}"))
            })
    }
    ...
    async fn handle_command(&mut self, command: AccountCommand) {
        ...
        match command {
            ...
            AccountCommand::RegisterOfflineWatch(result_tx, offline_watch) => {
                result_tx.send(self.handle_register_offline_watch(offline_watch));
            }
        };
    }
