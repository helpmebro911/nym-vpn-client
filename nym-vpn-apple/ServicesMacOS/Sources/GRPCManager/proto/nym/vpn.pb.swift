...
struct Nym_Vpn_NetworkCompatibility {
  // SwiftProtobuf.Message conformance is added in an extension below. See the
  // `Message` and `Message+*Additions` files in the SwiftProtobuf library for
  // methods supported on all messages.
...
struct Nym_Vpn_GetNetworkCompatibilityResponse {
  // SwiftProtobuf.Message conformance is added in an extension below. See the
  // `Message` and `Message+*Additions` files in the SwiftProtobuf library for
  // methods supported on all messages.
...
struct Nym_Vpn_TunnelState {
  ...
  enum BaseErrorStateReason: SwiftProtobuf.Enum {
    typealias RawValue = Int
    case firewall // = 0
    case routing // = 1
    case dns // = 2
  ...
  struct Error {
    ...
    enum OneOf_ErrorStateReason: Equatable {
    ...
    #if !swift(>=4.1)
      static func ==(lhs: Nym_Vpn_TunnelState.Error.OneOf_ErrorStateReason, rhs: Nym_Vpn_TunnelState.Error.OneOf_ErrorStateReason) -> Bool {
        // The use of inline closures is to circumvent an issue where the compiler
        // allocates stack space for every case branch when no optimizations are
        // enabled. https://github.com/apple/swift-protobuf/issues/1034
        switch (lhs, rhs) {
        case (.baseReason, .baseReason): return {
          guard case .baseReason(let l) = lhs, case .baseReason(let r) = rhs else { preconditionFailure() }
          return l == r
        }()
        case (.syncAccount, .syncAccount): return {
          guard case .syncAccount(let l) = lhs, case .syncAccount(let r) = rhs else { preconditionFailure() }
          return l == r
        }()
        case (.syncDevice, .syncDevice): return {
          guard case .syncDevice(let l) = lhs, case .syncDevice(let r) = rhs else { preconditionFailure() }
          return l == r
        }()
        case (.registerDevice, .registerDevice): return {
          guard case .registerDevice(let l) = lhs, case .registerDevice(let r) = rhs else { preconditionFailure() }
          return l == r
        }()
        case (.requestZkNym, .requestZkNym): return {
          guard case .requestZkNym(let l) = lhs, case .requestZkNym(let r) = rhs else { preconditionFailure() }
          return l == r
        }()
        case (.requestZkNymBundle, .requestZkNymBundle): return {
          guard case .requestZkNymBundle(let l) = lhs, case .requestZkNymBundle(let r) = rhs else { preconditionFailure() }
          return l == r
        }()
        default: return false
        }
...
extension Nym_Vpn_TunnelState.BaseErrorStateReason: CaseIterable {
  // The compiler won't synthesize support with the UNRECOGNIZED case.
...
extension Nym_Vpn_NetworkCompatibility: @unchecked Sendable {}
extension Nym_Vpn_GetSystemMessagesResponse: @unchecked Sendable {}
extension Nym_Vpn_GetNetworkCompatibilityResponse: @unchecked Sendable {}
...
extension Nym_Vpn_TunnelState.BaseErrorStateReason: @unchecked Sendable {}
...
extension Nym_Vpn_TunnelState.Error.OneOf_ErrorStateReason: @unchecked Sendable {}
